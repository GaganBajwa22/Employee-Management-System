...
import { Shield, Mail, KeyRound, Palette } from "lucide-react"
    ...
    <div className="space-y-8 max-w-4xl pb-10">
      <div>
        ...
        <p className="text-muted-foreground mt-2 text-sm">
          Manage your account preferences and admin profile.
        </p>
      </div>
        ...
        <section className="rounded-xl border border-border/50 bg-card/50 backdrop-blur-sm overflow-hidden shadow-sm">
          <div className="p-6 flex flex-col sm:flex-row items-start sm:items-center gap-6 border-b border-border/50">
            <Avatar className="h-24 w-24 border-2 border-primary/20 shadow-sm">
              <AvatarFallback className="bg-gradient-to-br from-indigo-500/20 to-purple-500/20 text-indigo-700 dark:text-indigo-300 text-3xl font-bold uppercase">
          ...
          <div className="p-6 bg-muted/20 space-y-4">
            <div className="bg-card border border-border/50 p-4 rounded-lg flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">Managed via Supabase secure authentication layer.</p>
              </div>
              <div className="px-3 py-1 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs font-semibold rounded-full border border-emerald-500/20">
                Secure
              </div>
            </div>
          </div>
